    ...
    try:
        for line in iter(process.stdout.readline, ''):
            if line:
                ...
                # Extract accuracy (case-insensitive)
                if "accuracy:" in line.lower():
                    try:
                        # Find the position of "accuracy:" (case-insensitive)
                        lower_line = line.lower()
                        acc_pos = lower_line.find("accuracy:")
                        if acc_pos != -1:
                            # Extract the part after "accuracy:"
                            acc_str = line[acc_pos + 9:].strip()  # 9 = len("accuracy:")
                            accuracy = float(acc_str)
                            output_queue.put(('accuracy', client_id, accuracy))
                    except (ValueError, IndexError) as e:
                        print(f"[MAIN] Error parsing accuracy for client {client_id}: {e}")
                        continue
                ...
                if use_dp and "privacy spent:" in line.lower():
                    try:
                        ...
                        print(f"[MAIN] Error parsing privacy metrics for client {client_id}: {e}")
                        continue
...
def save_results(results, csv_path, json_path, use_dp=True):
    ...
    if results:
        ...
        summary["final_metrics"]["avg_accuracy"] = sum(final_accuracies.values()) / len(
            final_accuracies) if final_accuracies else 0
